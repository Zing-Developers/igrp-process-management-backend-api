...
sem sessão IRN: o super admin mapeia o email do service account às permissões `MODULO:acao` na
consola `/email-access-mappings` (spec: `docs/SPEC_EMAIL_ACCESS_MAPPING.md`). Sem pepper, sem env
nova. O que devops garante no Keycloak:
...
| (sem variável) | — | **24.9, as duas apps: mapeamento de acesso por email** (`/email-access-mappings`, super admin). Sem env nova nem segredo: a única exigência é o realm Keycloak, ver secção 1b. |
