...
- O **super admin** é identificado por email igual a `IRN_API_SUPER_ADMIN_EMAIL` (trim, case-insensitive) —
  passa em todas as regras sem precisar de permissões. A partir da **24.8** o claim `email` do JWT é
  verificado **primeiro**: se bater, é super admin **sem cookie `session_id`** (o `/Auth/me` não é
  consultado); só se não bater é que se cai no email do `/Auth/me` da sessão, como antes.
...
| `IRN_API_SUPER_ADMIN_EMAIL` | email do super admin | **24.8**: comparado (trim, case-insensitive) primeiro com o claim `email` do JWT — bate → super admin sem sessão IRN; senão com o email do `/Auth/me` |
